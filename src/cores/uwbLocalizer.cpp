...
UWBLocalizer::UWBLocalizer() : Node("uwb_localizer_node") {
  ...
  mark_timer_ = this->create_wall_timer(
      100ms, std::bind(&UWBLocalizer::timer_callback, this));
  ...
  sonar_publisher_ =
      this->create_publisher<sensor_msgs::msg::Range>("/sonar/range", 10);

  drone_pose_subscriber_ =
      this->create_subscription<geometry_msgs::msg::PoseStamped>(
          "/nav/localState", 10,
          std::bind(&UWBLocalizer::setCurrentPose, this, _1));
  ...
  RCLCPP_INFO(
      this->get_logger(),
      "Success initializing UWB Localizer Node and %d Anchors Positions.",
      num_anchors_);
}

void UWBLocalizer::multilateration(
  ...
  // TODO : 경량 NLOS 식별이 녹아있음, 현재 위치와 앵커 정보를 바탕으로 거리
  // 임계를 설정하고, 그 임계값을 벗어나는 앵커는 제거하는 방식으로 구현되어
  // 있음.
  for (int i = 0; i < (int)msg->dist.size(); i++) {
    dist[i] = msg->dist[i] * 1e-2;
    anc_id[i] = msg->anchor_ids[i] - 1;
    temp_res.push_back(dist[i] -
                       (anchor_positions_[anc_id[i]] - opt_pos).norm());
  }
  ...
  int count = std::count_if(temp_res.begin(), temp_res.end(),
                            [](float d) { return d < 2.0; });
  ...
  // if (dist.size() >= 4)
  {
    ...
    for (int opt = 0; opt < iter; opt++) {
      ...
      for (int i = 0; i < count; i++) {
        ...
        res(i) =
            dist[idx[i]] - (anchor_positions_[anc_id[idx[i]]] - opt_pos).norm();
      }
...
void UWBLocalizer::setCurrentPose(
