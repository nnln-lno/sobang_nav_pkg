...
namespace navigation
{
  ...
  void Navigation::timer_callback()
  {
    ...
    {
      RCLCPP_INFO_ONCE(this->get_logger(), "Radar sensor is aborted.. No more navigation topic is published !");
      has_problems_ = true;
    }
    ...
    else if (count_ % 40 == 0 && imu_cnt == 0 && init_alignment_) // Print alignment status every 10 timer callbacks (5 seconds) during initial alignment                                                                         // 
    {
      RCLCPP_INFO(this->get_logger(), "Waiting for IMU data ..."); 
    }
    else if (count_ % 40 == 0 && imu_cnt > 0 && init_alignment_) // Print stop status every 10 timer callbacks (5 seconds) when drone is stationary
    {
