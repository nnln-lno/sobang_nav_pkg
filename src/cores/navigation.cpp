...
Navigation::Navigation() : Node("sobang_navigation_node"), count_(0) {
  ...
  ego_vel_publisher_ = this->create_publisher<geometry_msgs::msg::TwistStamped>(
      "/radar/egovel", 10);
 
  icp_state_publisher_ =
      this->create_publisher<geometry_msgs::msg::PoseStamped>("/icp/pose", 10);
...
void Navigation::imu_callback(const sensor_msgs::msg::Imu::SharedPtr i_msg) {
  ...
  Vec3d w_b = Vec3d{msg->angular_velocity.x, msg->angular_velocity.y,
                    msg->angular_velocity.z} - getState().gyro_bias;
...
void Navigation::radar_callback(
  ...
  if (icp_cnt % 10 == 0) {
    ...
    if (radar_estimator_.simpleRadar2DIcp(rel_R, rel_t)) {
      ...
      icp_pos_sum += Vec3d{icp_result.position(0), icp_result.position(1),
                           quat2euler(icp_result.quaternion)(2)};
      ...
      {
        z_att = (th_) / (2.0 * sin(th_)) *
                Vec3d{z_R(2, 1) - z_R(1, 2), z_R(0, 2) - z_R(2, 0), z_R(1, 0) - z_R(0, 1)};
      }      
...
void Navigation::uwbRangeCallback(
  ...
  measurementUpdate(
      getState(), residual, Hk, R_uwb_range * MatXd::Identity(count, count)); // [HYPERPARAM] UWB range measurement noise covariance
}
...
void Navigation::measurementUpdate(const drState predicted_state,
  ...
  if (!init_alignment_) {
    ...
    int n = P_full.rows();
    P_full = (MatXd::Identity(n,n) - K * Hk) * P_full *
             (MatXd::Identity(n,n) - K * Hk).transpose() +
         K * Rk * K.transpose();
                        ...
                        // initial alignment
  {
    if (view_state_) {
      std::cout << std::fixed << std::setprecision(4)
                ...
                << "], Gyro Bias (rad/s): ["
                << getState().gyro_bias.transpose()
                << "], Scale: ["
                << getState().scale.transpose() << std::endl;
      std::cout << "], Position Variance: ["
                << Pk.block(0, 0, 3, 3).diagonal().transpose()
                << "], Attitude Variance: ["
                << Pk.block(3, 3, 3, 3).diagonal().transpose() << "]"
                << std::endl;
    }
  } else if (count_ % 50 == 0 && imu_cnt == 0 &&
...
Vec2d Navigation::ahrs(Vec3d acc_accum) {
  ...
  double theta = std::atan2(-acc_accum(0),
                            std::sqrt(acc_accum(1) * acc_accum(1) +
                                      acc_accum(2) * acc_accum(2))); // Pitch
  ahrs_result << phi, theta;
  return ahrs_result;
}
...
void Navigation::printStateInfo() {
  ...
  std::cout << std::endl;
  std::cout
      << "[INFO] CHECK DRONE STATE BY SUBSCRIBING '/fmu/in/Vehicle_Visual_Odometry' TOPIC !"
      << std::endl;
  std::cout << std::endl;
  std::cout << "---------------------------------------------------------------"
               "---------"
...
void Navigation::param_setting() {
  ...
  this->declare_parameter("icp_cov",
                          std::vector<double>{1.5, 1.5, 1e3, 1e3, 1e3, 5});                          
  ...
  std::vector<double> init_acc_bias_vec =
      this->get_parameter("init_acc_bias").as_double_array();
  ...
  std::vector<double> icp_cov_vec =
      this->get_parameter("icp_cov").as_double_array();
  ...
  init_acc_bias_ = Vec3d{init_acc_bias_vec[0], init_acc_bias_vec[1],
                         init_acc_bias_vec[2]};
