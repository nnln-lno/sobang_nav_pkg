...
namespace navigation
{
  Navigation::Navigation() : Node("sobang_navigation_node"), count_(0)
  {
    ...
    {
      px4_sonar_subscriber_ = this->create_subscription<px4_msgs::msg::DistanceSensor>(
         sonar_topic_, qos_profile, std::bind(&Navigation::px4_sonarCallback, this, _1));
    }
    else
    {
  ...
  void Navigation::px4_sonarCallback(const px4_msgs::msg::DistanceSensor::SharedPtr msg)
  {
    ...
    if (sonar_range >= 7.649999)
    {
       return;
    }
    else if (sonar_range <= 0.2001)
    {
       sonar_range = 0.0;
    }
  ...
  void Navigation::ros2_sonarCallback(const sensor_msgs::msg::Range::SharedPtr msg)
  {
    ...
    // Mat3d dcm_now = quat2dcm(getState().quaternion);
    // double pse_hx  = ( -(getState().position.z() - tis.z()) ); 

    // std::cout << "Raw : " << sonar_range << ", Comp X : " << pse_hx << ", Comp O : " << pse_hx/dcm_now(2,2) << std::endl;
  ...
  void Navigation::timer_callback()
  {
    ...
    if (!init_alignment_ && radar_valid >= 10)
    {
        has_problems_ = true;
    }
    // RCLCPP_INFO(this->get_logger(), "Publishing Vehicle Odometry: count %d", msg.count);
    
    if (count_ % 20 == 0 && !init_alignment_) // Print state every 4 timer callbacks (2 seconds) after initial alignment
    {
      if (view_state_)
      { 
    ...
    else if (count_ % 50 == 0 && imu_cnt == 0 && init_alignment_) // Print alignment status every 10 timer callbacks (5 seconds) during initial alignment
    {
      RCLCPP_INFO(this->get_logger(), "Waiting for IMU data ..."); 
    }
    else if (count_ % 50 == 0 && imu_cnt > 0 && init_alignment_) // Print stop status every 10 timer callbacks (5 seconds) when drone is stationary
    {
